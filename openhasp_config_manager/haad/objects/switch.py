...
class SwitchObjectController(ObjectController):
    ...
    async def _setup_state_listener_and_sync(self):
        ...
        if self._entity_id is not None:
            await self.controller.react_to(
                ...
                action=self.__state_update_callback,
                immediately=True,
            )
    ...
    async def __state_update_callback(self, event: HaadStateEvent):
        enabled = True
        val = 0
        if event.new.lower() == SwitchState.ON:
            val = 1
        elif event.new.lower() == SwitchState.OFF:
            val = 0
        else:
            enabled = False
